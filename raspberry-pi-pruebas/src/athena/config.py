...
class GeometryConfig:
    ...
    Sirve para decidir cuándo FRENAR (``decision._perseguir``), nada más.
    Sin ToF (VL53L1X): se retiró del ESP32 junto con el bus I2C nº0 (ver el
    aviso grande en firmware-esp32/src/main.cpp) porque nunca dio una
    conexión física confiable en banco. El cierre de la pinza
    (``decision._aproximar_bandera``) YA NO usa esta distancia estimada --
    usa ``Detection.area_fraccion`` (qué fracción del frame ocupa la caja
    de la bandera, ver ``ControlConfig.area_agarre_fraccion``), que no
    depende de ``focal_px`` ni de ninguna calibración de cámara. Que
    ``focal_px`` esté sin calibrar sigue degradando el frenado, pero ya no
    puede hacer que el robot cierre la pinza lejos o al aire.
...
class ControlConfig:
    ...
    # Fracción del frame (0.0-1.0) que la caja de la bandera tiene que
    # ocupar para considerarla al alcance y cerrar la pinza. Reemplaza a la
    # antigua distancia en milímetros: esta no depende de ``focal_px`` ni de
    # ninguna calibración de cámara (ver el docstring de ``GeometryConfig``
    # y ``Detection.area_fraccion`` en types.py), solo de contar píxeles de
    # la propia caja contra el tamaño del frame -- útil porque, sin ToF, ya
    # no hay ninguna medición física de distancia real. 0.60 = pedido
    # explícito; A CALIBRAR EN BANCO según qué tan grande se ve la bandera
    # justo antes de que el gripper la alcance.
    area_agarre_fraccion: float = 0.60

    # Cuántos CUADROS seguidos tiene que sostenerse una detección antes de
    # actuar sobre ella -- filtra ruido de un cuadro suelto (una lectura de
    # color, una caja que por un instante se ve más grande o más chica de lo
    # normal). ~50 ms a 30 FPS, mismo criterio de debounce ya confirmado en
    # banco para el firmware standalone (ver su historial de commits). Se
    # usa para: llegar a la zona neutra, llegar a la zona propia, y decidir
    # que la bandera ya está al alcance para cerrar la pinza.
    frames_debounce_deteccion: int = 2
