...
| Cuándo **cerrar la pinza** | Qué fracción del frame ocupa la caja de la bandera (`Detection.area_fraccion`). Sin ToF (retirado del ESP32 junto con el bus I2C nº0), no hay ninguna medición física de distancia. |
...
- **Calibrar la focal** de la cámara (`GeometryConfig.focal_px` es hoy una
  estimación de webcam genérica). Afecta el frenado al acercarse
  (`decision._perseguir`), no el cierre de la pinza -- eso lo decide
  `Detection.area_fraccion`, que no depende de `focal_px`.
- **Calibrar `ControlConfig.area_agarre_fraccion`** (0.60 por ahora, pedido
  explícito sin verificar en banco): qué fracción del frame debe ocupar la
  bandera para cerrar la pinza. Sin ToF que mida de verdad, es el único
  número que decide cuándo agarrar -- probar en cancha con la bandera real
  y ajustar si cierra muy lejos o muy cerca.
