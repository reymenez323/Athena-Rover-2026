...
aviso grande en firmware-esp32/src/main.cpp) porque nunca dio una conexión
física confiable en banco. Saber cuándo cerrar la pinza sobre la bandera ya
no depende de ninguna medición física de distancia -- depende de qué
fracción del frame ocupa la caja de la bandera (``Detection.area_fraccion``,
calculado en ``_perception_from_box`` más abajo), ver
``decision._aproximar_bandera``.
...
def _perception_from_box(
    ...
    ``distance_mm`` es una ESTIMACIÓN por tamaño aparente (necesita
    ``focal_px`` calibrado), buena para decidir cuándo frenar
    (``decision._perseguir``) pero no para disparar el agarre: sin ToF
    (VL53L1X, retirado del ESP32 junto con el bus I2C nº0), lo que decide
    ``decision._aproximar_bandera`` es ``area_fraccion`` -- qué fracción del
    frame ocupa la caja de la bandera -- que no depende de ninguna
    calibración de cámara, solo de contar píxeles.
    """
    if frame_width <= 0:
        return Perception(
    ...
    # A diferencia de distance_mm, NO se descarta cerca del borde del frame:
    # una bandera que empieza a salirse del cuadro por lo grande que se ve es
    # justo la señal de "ya está muy cerca", no un motivo para desconfiar.
    area_fraccion = box.area / (frame_width * frame_height)

    deteccion = Detection(
        cls=cls_objetivo, box=box, confidence=confidence,
        distance_mm=distance_mm, area_fraccion=area_fraccion,
        angle_deg=angle_deg, track_id=1,
    )
    return Perception(
        ...
        with Camera(cfg.camera) as cam, \
            ...
            while not _parar:
                ...
                if cv2 is not None:
                    ...
                    # Sin ToF: se muestra qué fracción del frame ocupa el
                    # objetivo, que es lo que decide _aproximar_bandera en
                    # vez de una distancia física medida.
                    objetivo_vista = percepcion.best(decisor.state.bandera_objetivo)
                    if objetivo_vista is not None and objetivo_vista.area_fraccion is not None:
                        area_txt = f"{objetivo_vista.area_fraccion * 100:.0f}% del frame"
                        cv2.putText(vista, f"Area: {area_txt}", (8, 32),
